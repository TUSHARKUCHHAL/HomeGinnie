...
const ResetPassword = () => {
  ...
  useEffect(() => {
    if (success) {
      const timer = setTimeout(() => navigate("/ServiceProvider-Login"), 3000);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Use API_BASE_URL from environment or set it directly
      const API_BASE_URL = "http://localhost:5500";
      
      // Make sure the URL matches your backend route exactly
      const response = await fetch(`${API_BASE_URL}/api/service-providers/reset-password/${token}`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-slate-50 relative overflow-hidden">
      {/* Animated Blobs - No changes needed here */}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div id="password-requirements" className="bg-slate-100 rounded-md border border-slate-200 p-4">
              ...
              <ul className="space-y-1 pl-1">
                <li className={`text-xs flex items-center space-x-2 ${newPassword.length >= 8 ? "text-slate-900" : "text-slate-500"}`}>
                  <span>{newPassword.length >= 8 ? "●" : "○"}</span>
                  <span>At least 8 characters</span>
                </li>
                <li className={`text-xs flex items-center space-x-2 ${/[A-Z]/.test(newPassword) ? "text-slate-900" : "text-slate-500"}`}>
                ...
                <li className={`text-xs flex items-center space-x-2 ${/\d/.test(newPassword) ? "text-slate-900" : "text-slate-500"}`}>
                  <span>{/\d/.test(newPassword) ? "●" : "○"}</span>
                  <span>One number</span>
                </li>
                <li className={`text-xs flex items-center space-x-2 ${/[!@#$%^&*(),.?":{}|<>]/.test(newPassword) ? "text-slate-900" : "text-slate-500"}`}>
