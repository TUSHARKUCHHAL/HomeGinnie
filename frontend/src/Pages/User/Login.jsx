...
const LoginPage = () => {
  ...
  const parseErrorResponse = (error) => {
    // Check for specific error codes from backend and provide user-friendly messages
    if (error.response) {
      ...
      switch (status) {
        ...
        default:
          return data.message || 'An error occurred during login. Please try again.';
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Use our error parser to get a user-friendly message
      const errorMessage = parseErrorResponse(error);
      
      // Check if it's an invalid credentials error
      if (error.response && error.response.status === 401) {
        // For invalid credentials, we highlight both fields
  ...
  const handleGoogleLogin = async (credentialResponse) => {
    ...
    try {
      ...
      // Set success message
      setSuccess(userData.message || 'Google login successful! Redirecting...');
      ...
      // Use our error parser for Google login errors too
      ...
      <div className="w-full max-w-md z-10">
        ...
        <div className="bg-white/95 backdrop-blur-sm mt-28 mb-12 rounded-xl shadow-md border border-slate-200 overflow-hidden">
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6" onChange={clearServerError}>
              {/* Email Field */}
              <div>
                <label htmlFor="email" className="block text-sm font-medium text-slate-700 mb-1">
                  Email address
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <FaEnvelope className={`h-5 w-5 ${touched.email && errors.email ? 'text-red-400' : 'text-slate-400'}`} />
                  </div>
                  <input
                    id="email"
                    name="email"
                    ...
                    } bg-white py-3 shadow-sm focus:outline-none focus:ring-1 sm:text-sm transition-all`}
                    placeholder="you@example.com"
              ...
              <div>
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <FaLock className={`h-5 w-5 ${touched.password && errors.password ? 'text-red-400' : 'text-slate-400'}`} />
                  </div>
                  <input
                    id="password"
                    name="password"
